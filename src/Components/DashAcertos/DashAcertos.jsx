...
class DashAcertos extends Component {
  ...
  handleSubmit(event) {
    ...
    let newErrorMessage = "";
    if ( this.state.newSettle.paidBy === "" || this.state.newSettle.paidTo === "" || this.state.newSettle.value === 0) {
      newErrorMessage = "Por favor, preencha todos os dados";
        ...
        this.setState({ newSettle: copyNewSettle }, () => { console.log("this.state.errorMessage" ) });
      }
      this.setState({ errorMessage: newErrorMessage }, () => { console.log("this.state.errorMessage" ) });
    }
  render() {
    return (
      <>
          ...
          <form className="dashAddBills d-flex justify-content-between align-items-end flex-wrap px-4">
            <div className="form-group text-left col-lg-3 mt-1 mb-0 p-0">
              ...
              <select
                ...
                required
              >
                <option defaultValue>Selecione um nome</option>
                {[...this.state.members]
